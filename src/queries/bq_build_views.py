...
def create_violation_code_sheet_table():
    dataset_id = "hygiene-prediction-434.HygienePredictionRow"
...
CREATE OR REPLACE VIEW `hygiene-prediction-434.HygienePredictionRow.violation_code_count` AS
SELECT 
  code, 
...
CREATE OR REPLACE VIEW `hygiene-prediction-434.HygienePredictionRow.violation_code_count_description` AS
SELECT 
  c.code,
  s.description,
  c.violation_count
FROM `hygiene-prediction-434.HygienePredictionRow.violation_code_count` AS c
LEFT JOIN `hygiene-prediction-434.HygienePredictionRow.violation_code_sheet` AS s
ON c.code = s.code
"""

# View 3: violation_code_by_facility_category
violation_by_facility_category_sql = """
CREATE OR REPLACE VIEW `hygiene-prediction-434.HygienePredictionRow.violation_code_by_facility_category` AS
WITH exploded AS (
  SELECT 
    code,
    facility_category
...
CREATE OR REPLACE VIEW `hygiene-prediction-434.HygienePredictionRow.violation_correlation_view` AS
...
CREATE OR REPLACE VIEW `hygiene-prediction-434.HygienePredictionRow.violation_facility_labeled` AS
WITH exploded AS (
  SELECT 
    code,
    facility_category
...
CREATE OR REPLACE VIEW `hygiene-prediction-434.HygienePredictionRow.violation_cooccurrence_labeled_top10` AS
WITH top_codes AS (
  SELECT code
  FROM `hygiene-prediction-434.HygienePredictionRow.violation_code_count`
  ORDER BY violation_count DESC
  LIMIT 10
),
filtered AS (
  SELECT *
  FROM `hygiene-prediction-434.HygienePredictionRow.violation_correlation_view`
...
joined AS (
  ...
  FROM filtered AS f
  LEFT JOIN `hygiene-prediction-434.HygienePredictionRow.violation_code_sheet` AS sa
    ON f.code_a = sa.code
  LEFT JOIN `hygiene-prediction-434.HygienePredictionRow.violation_code_sheet` AS sb
    ON f.code_b = sb.code
)
SELECT *, 
...
CREATE OR REPLACE VIEW `hygiene-prediction-434.HygienePredictionRow.violation_by_zip` AS
SELECT 
  zip,
  code,
  COUNT(*) AS violation_count
...
CREATE OR REPLACE VIEW `hygiene-prediction-434.HygienePredictionRow.violation_by_facility` AS
WITH exploded AS (
  SELECT 
    dba_name,
    zip,
...
# CREATE OR REPLACE VIEW `hygiene-prediction-434.HygienePredictionRow.violation_trends_by_month_labeled` AS
# SELECT 
#     t.month,
#     t.code,
#     s.description,
...
# FROM `hygiene-prediction-434.HygienePredictionRow.violation_trends_by_month` t
...
CREATE OR REPLACE VIEW `hygiene-prediction-434.HygienePredictionRow.violation_by_facility_map` AS
SELECT 
  dba_name,
  address,
  latitude,
