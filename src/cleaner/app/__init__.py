...
def cleaner_1_drop(df: pl.DataFrame, logger=None) -> pl.DataFrame:
    ...
    columns_to_drop = ["aka_name", "license_", "latitude", "longitude", "location"]
...
def cleaner_2_inspection_id(df: pl.DataFrame, logger=None) -> pl.DataFrame:
    ...
    before = df.height
    df = df.filter(~pl.col("inspection_id").is_duplicated())
    df = df.filter(pl.col("inspection_id").str.contains(r"^\\d{7}$"))
    after = df.height
    log(f"Removed {before - after} rows with bad or duplicate inspection_id")
    return df
    ...
    if logger:
        for col in text_cols:
            if col in df.columns:
                ...
                logger.info(f"Column Normalization Summary: {col:<20} | Unique after: {after.item()}")
    return df
...
def cleaner_8_geolocation(df: pl.DataFrame, logger=None) -> pl.DataFrame:
    ...
    df = df.filter(
        pl.col("zip").is_not_null() & (pl.col("zip").str.lengths() == 5)
    )
